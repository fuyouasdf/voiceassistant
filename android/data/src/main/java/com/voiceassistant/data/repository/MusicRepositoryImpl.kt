...
import com.voiceassistant.data.remote.Song as RemoteSong
...
class MusicRepositoryImpl(
    ...
    override suspend fun searchSongs(query: String): Result<List<Song>> = withContext(Dispatchers.IO) {
        try {
            ...
            if (response.isSuccessful) {
                val remoteSongs = response.body()?.response?.searchResult3?.song ?: emptyList()
    ...
    private fun RemoteSong.toDomainModel(): Song {
        return Song(
            id = id,
            title = title,
            artist = artist,
