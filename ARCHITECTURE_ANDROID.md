...
- 播放页支持歌词区：优先读取 Jellyfin `GET /Audio/{itemId}/Lyrics` 返回的时间轴歌词，若服务端返回普通 `LRC` 文本也可兼容解析；歌词以列表形式展示，当前行会高亮放大并自动滚动到可视区域中部，用户可点击歌词行直接跳转到对应时间点，无歌词时会明确提示。
- 点击播放页歌词卡可进入全屏纯歌词模式 `LyricsFullscreenActivity`，该页面保留歌曲信息和播放时间，优先提供大字号歌词阅读与点击跳转。
- 歌词高亮切换采用轻量淡入与缩放过渡，避免旧设备上出现明显卡顿，同时提升当前行切换的阅读连续性。
- 用户手动滚动歌词列表时，自动居中跟随会暂时暂停；待列表回到空闲状态后，再恢复到当前播放行，避免阅读时被自动滚动抢焦点。
- `QueueActivity` 负责展示当前播放队列，支持查看当前曲目、点击切歌、长按拖拽排序，以及将歌曲从当前本机队列移除；该操作只影响当前播放队列，不修改用户保存的播放列表。
- 播放页与队列页的核心操作按钮统一使用项目内自定义矢量图标，不再混用 Android 系统图标，保证视觉风格一致。
