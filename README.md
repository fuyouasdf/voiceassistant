...
**这是一款专为家庭音乐爱好者设计的局域网语音控制应用。**

### 典型使用流程

```
┌──────────────┐    语音命令     ┌──────────────────┐    HTTP/DLNA    ┌──────────────┐
│  旧手机       │ ────────────→ │  Voice Assistant │ ──────────────→ │  Jellyfin   │
│  (语音入口)   │   "播放音乐"   │   (本地运行)     │    获取音乐     │   (NAS)      │
└──────────────┘                └──────────────────┘                 └──────────────┘
                                       │
                                       │ DLNA 推送
                                       ↓
...
1. **客厅音响控制** - 躺在沙发上说"播放周杰伦的歌"，音乐从 NAS 流到功放
2. **卧室音乐时光** - 旧手机放在床头，语音控制"上一首"、"暂停"
3. **书房背景音乐** - 工作时语音点歌，声音从书房的有源音箱输出
4. **派对模式** - 对着手机说"播放欢快的歌"，NAS 上的派对歌单开始播放
...
| 要求 | 说明 | 必选 |
|------|------|------|
| Android 设备 | Android 8.0+ (API 26)，推荐备用旧手机 | ✅ |
| Jellyfin 服务器 | 音乐库部署在家中 NAS，同一局域网 | ✅ |
| DLNA 播放设备 | 支持 DLNA/UPnP 的音响、TV、播放器 | ✅ |
| 局域网 | 设备在同一 WiFi 下 | ✅ |
| 本地模型（可选） | 内置中文 ASR/TTS，无需联网 | ❌ |
...
| 对比项 | 商业方案 (小爱/小度) | 本项目 |
|--------|---------------------|--------|
| 隐私 | 语音数据上传服务器 | 100% 本地处理 |
| 网络依赖 | 必须联网 | 完全离线可用 |
| 音乐控制 | 仅支持自家平台 | Jellyfin + DLNA |
| 灵活性 | 受限生态 | 完全开源定制 |
| 成本 | 需购买指定设备 | 用旧手机即可 |
...
- 🔇 **完全离线** - 语音识别、唤醒词、TTS 全部本地运行，不上传数据
- 🎵 **Jellyfin 集成** - 直接从 Jellyfin 播放音乐
- 📻 **DLNA 推送** - 把音乐推送到支持 DLNA 的设备播放
- 🎤 **本地 ASR/TTS** - 基于 Sherpa-ONNX，无需网络
- 📱 **后台运行** - 前台服务 + 电池优化白名单
- 🎨 **小爱风格 UI** - 流体渐变动画 + 直观状态反馈
